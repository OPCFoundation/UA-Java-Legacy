...
public class EndpointUtil {
   ...
   * </p>
   *
   * @param userIdentityTokens an array of {@link org.opcfoundation.ua.core.UserTokenPolicy}
   *        objects.
   * @return a boolean.
   */
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if endpoint or the stack doesn't
   *         support Anonymous token policy
   */
  public static UserIdentityToken createAnonymousIdentityToken(EndpointDescription ep) throws ServiceResultException {
    ...
    if (policy == null) {
      throw new ServiceResultException(StatusCodes.Bad_IdentityTokenRejected,
          "Anonymous UserTokenType is not supported");
    }
  ...
  public static UserIdentityToken createIssuedIdentityToken(EndpointDescription ep, ByteString byteString,
    ...
    if (algorithmUri == null) {
      algorithmUri = SecurityAlgorithm.RsaOaep;
    }
    try {
  ...
  public static UserIdentityToken createUserNameIdentityToken(EndpointDescription ep, ByteString byteString,
    ...
    if (algorithm == null) {
      ...
      try {
        ...
        if (byteString != null) {
          pw = ByteBufferUtils.concatenate(toArray(pw.length + byteString.getLength()), pw, byteString.getValue());
        } else {
   ...
   */
  public static X509IdentityToken createX509IdentityToken(EndpointDescription ep, byte[] serverNonce, Cert certificate,
      PrivateKey key, SignatureData signatureData) throws ServiceResultException {
    ...
    if ((securityPolicy != null) && (serverCert != null)) {
      try {
        ...
        // key
        Signature signature =
            Signature.getInstance(securityPolicy.getAsymmetricSignatureAlgorithm().getTransformation());
   ...
   *
   * @param enableIPv6 Set true to enable IPv6 addressing. Requires Java 7 or later on Windows
   *        platforms.
   * @return all Internet addresses of this computer
   * @throws java.net.SocketException if any.
   */
  public static Set<InetAddress> getInetAddresses(boolean enableIPv6) throws SocketException {
   ...
   */
  public static EndpointDescription select(EndpointDescription[] endpoints) throws ServiceResultException {
    // Filter out all but opc.tcp protocol endpoints
    exit: {
      EndpointDescription[] tcpEndpoints = EndpointUtil.selectByProtocol(endpoints, "opc.tcp");
      // Filter out all but Signed & Encrypted endpoints
      // tcpEndpoints = EndpointUtil.selectByMessageSecurityMode(tcpEndpoints,
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if any.
   */
  public static EndpointDescription select(EndpointDescription[] endpoints, String url) throws ServiceResultException {
   ...
   */
  public static EndpointDescription[] select(EndpointDescription[] searchSet, String url, String protocol,
    ...
    for (EndpointDescription d : searchSet) {
      ...
      if (protocol != null && !endpointUrl.startsWith(protocol.toLowerCase())) {
        continue;
      }
      if (url != null && !ObjectUtils.objectEquals(endpointUrl, url.toLowerCase())) {
        continue;
      }
      if (mode != null && !ObjectUtils.objectEquals(d.getSecurityMode(), mode)) {
        continue;
      }
      if (policy != null && !ObjectUtils.objectEquals(d.getSecurityPolicyUri(), policy.getPolicyUri())) {
        continue;
      }
      if (serverCertificate != null
          && !Arrays.equals(serverCertificate, ByteString.asByteArray(d.getServerCertificate()))) {
        continue;
      }
      result.add(d);
    }
   ...
   * <p>
   * Selection uses the following precedence: 1) Protocol must be opc.tcp (as http is not
   * implemented) 2) Security uses sign and encrypt 3) Select highest security level (determined by
   ...
   */
  public static List<SocketAddress> toSocketAddresses(String endpointUrl) throws IllegalArgumentException {
  ...
  public static boolean urlEqualsHostIgnoreCase(URI uri, URI requestedUri) {
    return uri.getScheme().equalsIgnoreCase(requestedUri.getScheme()) && uri.getPort() == requestedUri.getPort()
