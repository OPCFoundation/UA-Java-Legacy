/*
 * Copyright (c) 1996-2015, OPC Foundation. All rights reserved. The source code in this file is
 * covered under a dual-license scenario: - RCL: for OPC Foundation members in good-standing - GPL
 * V2: everybody else RCL license terms accompanied with this source code. See
 * http://opcfoundation.org/License/RCL/1.00/ GNU General Public License as published by the Free
 * Software Foundation; version 2 of the License are accompanied with this source code. See
 * http://opcfoundation.org/License/GPLv2 This source code is distributed in the hope that it will
...
public class Client {
   ...
   * {@link Application#addApplicationInstanceCertificate(KeyPair)}
   *
   ...
   * 
   * @param localEndpointUrl, The client opens a server socket based on this address, listens server
   *        sent ReverseHello
   ...
   */
  public SecureChannel createReverseSecureChannel(String localEndpointUrl, String serverApplicationUri,
      TransportChannelSettings settings) throws ServiceResultException {
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public SecureChannel createSecureChannel(String connectUrl, EndpointDescription endpoint)
      throws ServiceResultException {
    ...
    // );
    // settings.getOpctcpSettings().setClientCertificate(
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public SecureChannel createSecureChannel(String connectUrl, String endpointUri, SecurityMode mode,
      Cert remoteCertificate) throws ServiceResultException {
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public SecureChannel createSecureChannel(String connectUrl, TransportChannelSettings settings)
      throws ServiceResultException {
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public ServiceChannel createServiceChannel(String connectUrl, String endpointUri, SecurityMode mode,
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public ServiceChannel createServiceChannel(TransportChannelSettings settings) throws ServiceResultException {
   ...
   * @param maxResponseMessageSize max size of response messages - if null, use 4194304
   * @param requestedSessionTimeout requested session time out (in ms) - if null, use 3600000 (one
   *        hour)
  ...
  public Session createSession(SecureChannel channel, UnsignedInteger maxResponseMessageSize,
      Double requestedSessionTimeout, String sessionName) throws IllegalArgumentException, ServiceResultException {
   ...
   * @param requestedSessionTimeout requested session timeout (ms) - if null, defaulting to 3600000
   *        (one hour)
   * @param sessionName session name - if null, a random GUID is used to generate the name
   ...
   *        it does not match, an exception is thrown, if null the check is disabled
   * @return the session
  ...
  public Session createSession(SecureChannel channel, UnsignedInteger maxResponseMessageSize,
    ...
    if (sessionName == null) {
      sessionName = UUID.randomUUID() + "-" + String.format("%08X", System.identityHashCode(session));
    }
    ...
    session.serverCertificate = serverCert == null || serverCert.length == 0 ? null : new Cert(serverCert);
    ...
    session.serverSoftwareCertificates = res.getServerSoftwareCertificates();
    ...
    if (!MessageSecurityMode.None.equals(channel.getMessageSecurityMode())) {
      ...
      byte[] dataServerSigned = ByteBufferUtils.concatenate(ByteString.asByteArray(req.getClientCertificate()),
          ByteString.asByteArray(session.clientNonce));
      ...
      // certificate
      boolean ok = CryptoUtil.getCryptoProvider().verifyAsymm(session.serverCertificate.getCertificate().getPublicKey(),
          algorithm, dataServerSigned, ByteString.asByteArray(serverSignature.getSignature()));
      if (!ok) {
        throw new ServiceResultException(Bad_ApplicationSignatureInvalid,
     ...
     * i.e. Endpoint list from the GetEndpoint/Discovery service. Based on spec part 4: 5.6.2.1, the
     * client shall verify that EndpointDescriptions with matching transportProfileUri match.
     * 
     * Based on spec part 4: 5.6.2.2. (Table 11) we need to verify the following parameters:
     * endpointUrl, securityMode, securityPolicyUri, userIdentityTokens, transportProfileUri and
     * securityLevel. In case the endpoints differ, the client shall close the Session and report
     * error.
     */
    String uri = channel.getEndpointDescription().getTransportProfileUri();
    validateEndpoints(uri, endpoints, discoveredEndpoints);

    EndpointDescription[] filteredEndpoints = select(endpoints, endpoint.getEndpointUrl(), null,
        endpoint.getSecurityMode(), SecurityPolicy.getSecurityPolicy(endpoint.getSecurityPolicyUri()), null);
    ...
    if (filteredEndpoints.length == 0 && discoveredEndpoints == null) {
      filteredEndpoints = select(endpoints, null, null, endpoint.getSecurityMode(),
          SecurityPolicy.getSecurityPolicy(endpoint.getSecurityPolicyUri()), null);
    }
    if (filteredEndpoints.length == 0) {
      logger.error("Requested endpoint is not found on the server: Endpoint={}", endpoint);
   ...
   * <p>
   * To close the object, both secure channel and the session must be close separately.
   * SessionChannel.closeSession() SessionChannel.closeSecureChannel()
   *
   * @param applicationDescription application description
   * @return session channel
   * @throws org.opcfoundation.ua.common.ServiceResultException on errors
   */
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException on errors
   */
  public SessionChannel createSessionChannel(EndpointDescription endpoint) throws ServiceResultException {
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException on errors
   */
  public SessionChannel createSessionChannel(String connectUrl, EndpointDescription endpoint)
      throws ServiceResultException {
    // Create service channel
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public ApplicationDescription[] discoverApplications(String connectUrl, String discoverServerEndpointUri)
      throws ServiceResultException {
    ...
    try {
      FindServersRequest req = new FindServersRequest(null, discoverServerEndpointUri, new String[0], new String[0]);
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException Error that occured while processing
   *         the operation.
   */
   ...
   *
   * @param connectUrl address that contains the socket address to the endpoint
   * @param discoveryEndpointUri the endpoint identifier that identifies the endpoints that are
   *        being discovered
   * @return Endpoint Descriptions
   * @throws org.opcfoundation.ua.common.ServiceResultException Error that occured while processing
   *         the operation.
   ...
   * @throws org.opcfoundation.ua.common.ServiceResultException if error
   */
  public ServerOnNetwork[] discoverServersOnNetwork(String connectUrl, String discoverServerEndpointUri)
      throws ServiceResultException {
   ...
   */
  public ServerOnNetwork[] discoverServersOnNetwork(String connectUrl, String discoverServerEndpointUri,
      String[] serverCapabilities) throws ServiceResultException {
  ...
  public FindServersOnNetworkResponse discoverServersOnNetwork(String connectUrl, String discoverServerEndpointUri,
      UnsignedInteger startingRecordId, UnsignedInteger maxRecordsToReturn, String[] serverCapabilities)
      throws ServiceResultException {
    // Must not use encryption!
    SecurityMode mode = SecurityMode.NONE;
    ...
    ChannelService chan = new ChannelService(channel);
    try {
      FindServersOnNetworkRequest req =
          new FindServersOnNetworkRequest(null, startingRecordId, maxRecordsToReturn, serverCapabilities);
      req.setRequestHeader(new RequestHeader());
      req.getRequestHeader().setTimeoutHint(UnsignedInteger.valueOf(getTimeout()));
  ...
  /**
   * <p>
   * Getter for the field <code>endpointConfiguration</code>.
   * </p>
   *
  ...
  /**
   * <p>
   * Setter for the field <code>endpointConfiguration</code>.
   * </p>
   *
  ...
  private void validateEndpoints(String uri, EndpointDescription[] endpoints, EndpointDescription[] discoveredEndpoints)
      throws ServiceResultException {
    ...
    /*
     * We need to verify the following parameters: endpointUrl, securityMode, securityPolicyUri,
     * userIdentityTokens, transportProfileUri and securityLevel. Since .equals does compare all
     * these, lets use it. It also compares server ApplicationDescription and Certificate, so we
     * clone and set those to null
     */
    ...
    for (EndpointDescription e : dedl) {
      if (!edl.contains(e)) {
        logger.error(
            "The endpoint received from GetEndpoints is not in the endpoints of CreateSessionResponse. Endpoint={}", e);
  ...
  SecureChannel createSecureChannelImpl(String uri) throws ServiceResultException {
    ...
    if (proto.equals(UriUtil.SCHEME_OPCTCP)) {
      ...
      return channel;
    } else if (proto.equals(UriUtil.SCHEME_HTTPS)) {
    ...
    } else if (proto.equals(UriUtil.SCHEME_HTTP)) {
      HttpsClient client = new HttpsClient(proto);
      HttpsClientSecureChannel secureChannel = new HttpsClientSecureChannel(client);
      return secureChannel;
    } else {
